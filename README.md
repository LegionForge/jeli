...
> The diagram below is the conceptual model. For the code-level view, covering which module implements which branch and the write/read/verify paths step by step, see [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md), or take the [guided CodeTour](#guided-code-walkthrough-vs-code) in VS Code.
...
**Branches:**
- **Executive (Agents):** Hermes, Claude, Codex. Agents propose memories via MCP only; no direct DB access
- **Scoped MCP:** Jeli's enforcement point. It authenticates callers, caps trust on flagged content, logs every operation
...
- **Microsoft Security (Feb 2026):** "AI Recommendation Poisoning", with dozens of companies exploiting it at scale
...
| [docs/quarantine-lab-contract.md](docs/quarantine-lab-contract.md) | **Proposed for review**: testable storage, role, analyzer, and signed synthetic-batch invariants for issue #48 |
...
### Claude (Anthropic)
Much of this project's code and documentation was written in collaboration with [Claude](https://claude.com/claude-code), working under JP's direction. Design decisions, priorities, and the words that matter are JP's; the drafting, testing, and research legwork were shared. Every commit Claude contributed to carries a `Co-Authored-By` trailer, so the split is visible in the history.
