...
3. **Injection defense** (`security.py`) — regex patterns run over a
   ...
   never blocked — blocking would teach an attacker what evades. The
...
6. **Advisory-locked chain append** (`memory_tools.py::capture_memory`) —
   `prev_hash` read and INSERT are serialized under a Postgres advisory
...
override reason — the store can hold security documentation without
self-poisoning.
